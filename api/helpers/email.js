...
const getEmailToken = async () => {
  // Return the axios promise directly; behavior unchanged.
  return axios.post(
    _CHES_AUTH_ENDPOINT,
    qs.stringify({
  ...
  try {
    ...
    if (emailToken && emailToken.data && emailToken.data.access_token) {
      ...
      defaultLog.info('Email sent successfully via CHES', {
        to:
          emailTemplate.contexts && emailTemplate.contexts.length > 0
            ? emailTemplate.contexts[0].to
            : undefined,
    ...
    defaultLog.error('Email send via CHES failed in email helper', {
 ...
 */
exports.sendConfirmEmail = async (projectNames, email, confirmKey) => {
  const { projectList } = buildProjectEmailContext(projectNames);
  const emailTemplate = {
    bodyType: 'text',
    body:
      'Please confirm your email address to receive updates from Planning In Partnership.\r\n\r\n' +
 ...
 * @returns {Promise<void>}
 */
exports.sendWelcomeEmail = async (projectNames, email) => {
  const { projectList } = buildProjectEmailContext(projectNames);
  const emailTemplate = {
    bodyType: 'text',
    body:
      'Thank you for confirming your email address. You are now subscribed to updates for:\r\n' +
      '{{ projectList }}\r\n\r\n' +
 ...
 * @returns {Promise<void>}
 */
exports.sendProjectAddedEmail = async (projectNames, email) => {
  const { projectList } = buildProjectEmailContext(projectNames);
  const emailTemplate = {
    bodyType: 'text',
    body:
      'You have been added to updates for:\r\n' +
      '{{ projectList }}\r\n\r\n' +
      'You will receive project updates directly to your inbox.\r\n\r\n' +
      'If at any time you want to unsubscribe, click the link below:\r\n' +
      '{{ unsubcribeHost }}unsubscribe\r\n\r\n' +
 ...
 * @returns {Promise<void>}
 */
exports.handleContactFormResponse = async (
  projectName,
  contactFormResponse,
  recipients
) => {
  ...
  try {
    ...
    defaultLog.error('Email handle contact form response failed.', {
      status: e && (e.status || e.statusCode),
      err: { name: e && e.name, message: e && e.message, stack: e && e.stack }
    });
    throw e;
