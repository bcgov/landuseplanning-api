 ...
 */
exports.getDynamicSitemap = async (_, res) => {
  ...
  try {
    ...
    const [projects, commentPeriods] = await Promise.all([
      Project.find({
    ...
    projects.forEach((project) => {
      const lastModDate =
        project.dateUpdated || project.dateAdded || new Date();
    ...
    commentPeriods.forEach((commentPeriod) => {
      if (commentPeriod.project && commentPeriod.project._id) {
        const lastModDate =
          commentPeriod.dateUpdated || commentPeriod.dateAdded || new Date();
        const formattedDate = new Date(lastModDate).toISOString().split('T')[0];
