/**
 * This is a performance heavy version of updateShapes.js that blindly updates ALL ACRFD applications, and should only be used sparingly as needed.
 *
 * Fetches all ACRFD applications, in batches of 100.
 * For each application, fetches the matching Tantalis application, and updates the ACRDF application features and meta to match whatever is in Tantalis (the source of truth).
 *
 * Additionally, if an ACRFD application has reach a retired state, it unpublishes the application.
 */
...
var renewJWTLogin = function () {
  return new Promise(function(resolve, reject) {
    ...
    // less than 60 seconds left before token expiry
